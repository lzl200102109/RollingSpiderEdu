...
% function [sys,x0,str,ts] = ref_xy(t,x,flag)
%  
%     switch flag
%         case 0
...
% calibCycles   = 2;
% takeoffCycles = 1;
% waitCycles = calibCycles + takeoffCycles + 5;
% 
% if (t > waitCycles && t <= waitCycles + refCycles)
%     x_ref = 0;
%     y_ref = (t - waitCycles)/refCycles;
% elseif ((t > waitCycles + refCycles && t <= waitCycles + 2*refCycles))
%     x_ref = (t - waitCycles)/refCycles;
%     y_ref = 1;
% elseif ((t > waitCycles + 2*refCycles && t <= waitCycles + 3*refCycles))
%     x_ref = 1;
%     y_ref = 1 - (t - waitCycles)/refCycles;
% elseif ((t > waitCycles + 3*refCycles && t <= waitCycles + 4*refCycles))
%     x_ref = 1 - (t - waitCycles)/refCycles;
%     y_ref = 0;
% else
%     x_ref = 0;
%     y_ref = 0;
